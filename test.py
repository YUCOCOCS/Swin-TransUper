...
def inference(args, model, test_save_path=None):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        image, label, case_name = sampled_batch["image"].type(torch.FloatTensor).cuda(), sampled_batch["label"].type(torch.FloatTensor).cuda(), sampled_batch['case_name'][0]
    ...
    for i in range(1, args.num_classes):
        logging.info('Mean class %d mean_dice: %f mean_hd95: %f  SE: %f SP: %f ACC: %f' % (i, metric_list[i-1][0], metric_list[i-1][1],metric_list[i-1][2],metric_list[i-1][3],metric_list[i-1][4]))
    performance = np.mean(metric_list, axis=0)[0]
    mean_hd95 = np.mean(metric_list, axis=0)[1]
    mean_se = np.mean(metric_list,axis=0)[2]
...
if __name__ == "__main__":
    ...
    for i in range(170,200):
        args.ckpt_path = "/home/JianjianYin/transdeeplab/Synapse_4_6/swin_224_7_3level_epoch_%s.pth"%(i)
